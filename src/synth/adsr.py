...
class ADSREnvelope:
    ...
    def make_attack_decay_table(self):
        if self.curve == "expo":
            attack = 1 - np.exp(-self.k * (np.arange(self.sampling_rate*self.attack_time)/self.sampling_rate))
            decay = self.sustain_level + (1-self.sustain_level) * np.exp(-self.k * (np.arange(self.sampling_rate*self.attack_time)/self.sampling_rate))
            env_arr = np.concatenate([attack,decay])
            return env_arr

    def get_amplitude(self, sample_passed):
