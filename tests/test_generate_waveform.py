...
def test_return_type_from_generate_waveform(test_type):
    ...
    assert isinstance(res, tuple), f"{test_type}: Tuple should be returned from calling generate_waveform"
    assert isinstance(res[0], np.ndarray), f"{test_type}: Waveform returned from generate_waveform should be numpy array"
...
# 4. sampling rate working correctly 
# 5. duration working correctly 